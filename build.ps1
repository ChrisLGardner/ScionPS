...
Where-Object {$_.FullName -notlike '*Output*' -and $_.FullName -notlike '*Dependency*'} |
...
#Remove-Item -Path $OutputFolder -Recurse -Force -Confirm:$false
...
$Classes = Get-ChildItem -Path $SourceFolder -Include 'Class' -Recurse -Directory | Get-ChildItem -Include *.ps1 -File


foreach ($class in $Classes) {
    Get-Content -Path $Class.FullName | Add-Content -Path $DestinationModule -Encoding UTF8
}
...
Write-Verbose "Running tests found under $SourceFolder"

Invoke-Pester -Script $SourceFolder -Verbose:$False #-CodeCoverage $DestinationModule

Invoke-ScriptAnalyzer -Path $DestinationModule -Verbose:$False
